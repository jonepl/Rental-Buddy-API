...
from app.providers.rentcast.mapper import build_rentcast_params
...
class RentCastAdapter(ListingsPort):
    ...
    async def fetch_sales(self, request: SearchRequest) -> list[NormalizedListing]:
        """
        ...
        """
        params = build_rentcast_params(request)
        raw = await self.client.get_sales(params)
        return normalize_rentcast_response(raw, ContextRequest.SALES)

    async def fetch_rentals(self, request: SearchRequest) -> list[NormalizedListing]:
        """
        ...
        return normalize_rentcast_response(raw, ContextRequest.RENTALS)
